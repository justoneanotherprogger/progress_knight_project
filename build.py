...
export const I18N = {repr(locales)};
export const CHANGELOG = {repr(changelog)};
...
export let currentLang = localStorage.getItem('lang') || getBrowserLang();
...
def generate_data_files():
    ...
    for json_file in sorted(CONTENT_DIR.glob("*.json")):
        ...
        if container_name is not None:
            ...
            lines.append(f"export const {container_name} = {json.dumps(categories_container, ensure_ascii=False)};")
            if unified:
                lines.append(f"export const {entity_prefix}BaseData = Object.assign({{}}, ...Object.values({container_name}).map(c => c.items));")
            else:
                lines.append(f"export const {entity_prefix}BaseData = Object.assign({{}}, ...Object.values({container_name}));")
        else:
            for key, value in data.items():
                lines.append(f"export const {key} = {json.dumps(value, ensure_ascii=False)};")

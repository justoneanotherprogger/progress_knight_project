...
def generate_data_files():
    ...
    for json_file in sorted(CONTENT_DIR.glob("*.json")):
        ...
        if json_file.stem == "skills":
            # Categories are the source of truth; the flat dict is derived for the runtime.
...
def load_entity_names():
    """Extract legacy entity names, content skill ids and category names from data sources."""
    ...
    for json_file in sorted(CONTENT_DIR.glob("*.json")):
        with open(json_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        if json_file.stem == "skills" and "skillCategories" in data:
        ...
        for section in data.values():
            if not isinstance(section, dict):
                continue
            if all(isinstance(v, list) for v in section.values()):
                categories |= set(section.keys())
            else:
                legacy |= set(section.keys())
    return legacy, content_skills, categories
...
def validate(html: str, locales: dict, errors=None):
    ...
    entities, content_skills, categories = load_entity_names()
    ...
    for lang, keys in locales.items():
        ...
        for skill_id in sorted(content_skills):
            if (skill_id + "_name") not in keys:
                errors.append(f"[{lang}] Missing {skill_id}_name translation")
            if (skill_id + "_desc") not in keys:
                errors.append(f"[{lang}] Missing {skill_id}_desc translation")

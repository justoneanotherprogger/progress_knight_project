...
function serialize(gameData) {
	...
	for (const key of DECIMAL_FIELDS)
		if (!isDefaultValue(gameData[key]))
			dto[key] = decimalToString(gameData[key]);
	...
	dto.currentMisc = gameData.currentMisc
		.map((item) => item?.id)
		.filter((id) => id != null);
	...
	for (const key in gameData.requirements)
		dto.requirements[key] = gameData.requirements[key].completed
			? { completed: 1 }
			: {};
...
function peekSettingFromSave(setting) {
	try {
		...
		return coerceValue(
			gameDataSave.settings[setting],
			gameData.settings[setting],
		);
	} catch (error) {
		console.error(error);

...
function update() {
	...
	gameData.dark_orbs = gameData.dark_orbs.add(
		orbGeneration.mul(getGameSpeed() / updateSpeed),
	);
...
function updateStats() {
	...
	if (gameData.requirements.req_stats_essence_gain.isCompleted()) {
		...
		if (
			gameData.stats.EssencePerSecond.gt(gameData.stats.maxEssencePerSecond)
		) {
...
function autoPerks() {
	...
	if (
		...
		!isDecimalInfinity(gameData.dark_orbs)
	)
		buyDarkOrbGenerator();
	if (
		gameData.perks.auto_dark_orb === 1 &&
	...
	if (
		...
		if (
			gameData.hypercubes >
			evilTranCost() * PERK_AUTO_SACRIFICE_COST_MULTIPLIER
		)
			buyEvilTran();
		if (
			gameData.hypercubes >
			boostDurationCost() * PERK_AUTO_SACRIFICE_COST_MULTIPLIER
		)
			buyBoostDuration();
		if (
			gameData.hypercubes >
			reduceBoostCooldownCost() * PERK_AUTO_SACRIFICE_COST_MULTIPLIER
		)
			buyReduceBoostCooldown();
		if (
			gameData.hypercubes >
			hypercubeGainCost() * PERK_AUTO_SACRIFICE_COST_MULTIPLIER
		)
			buyHypercubeGain();
	}
}
...
function applyExpenses() {
	...
	if (gameData.coins.lt(0)) {
		...
		if (getIncome().lt(getExpense())) goBankrupt();
	}
}
...
function makeHeroes() {
	...
	for (const taskname in gameData.taskData) {
		...
		if (req instanceof TaskRequirement) {
			if (!req.isCompletedActual(true)) continue;
			for (const requirement of req.requirements)
				if (
					!(
						gameData.taskData[requirement.task] &&
						gameData.taskData[requirement.task].isHero
					)
				) {
					isNewHero = false;
					break;

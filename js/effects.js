...
function matchTarget(target, key, task) {
    ...
    switch (target.kind) {
        ...
        case "task": return target.task === key
        case "skill":
            return task instanceof Skill && (!target.category || key in skillCategories[target.category].items)
        case "job":
            return task instanceof Job && (!target.category || task.name in jobCategories[target.category].items)

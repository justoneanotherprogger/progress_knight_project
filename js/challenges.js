...
function getChallengeBonus(challenge_name, current = false) {
	...
	const val1 = current
		? getHappiness()
		: toInfinityNumber(gameData.challenges.an_unhappy_life);
	const val2 = current
		? getIncome()
		: toInfinityNumber(gameData.challenges.rich_and_the_poor);
	const val3 = current
		? getUnpausedGameSpeed() / baseGameSpeed
		: toInfinityNumber(gameData.challenges.time_does_not_fly);
	const val4 = current
		? getEvilGain().sub(10).max(0)
		: toInfinityNumber(gameData.challenges.dance_with_the_devil);
	const val5 = current
		? getChallengeTaskGoalProgress("job_chairman")
		: toInfinityNumber(gameData.challenges.legends_never_die);
	const val6 = current
		? getChallengeTaskGoalProgress("job_sigma_proioxis") / 100.0
		: toInfinityNumber(gameData.challenges.the_darkest_time);
...
function getChallengeGoal(challenge_name) {
	...
	if (challenge_name === "time_does_not_fly" || challenge_name === 3) {
		const goal = toChallengeDecimal(
			gameData.challenges.time_does_not_fly,
			0,
		).add(0.1);
		return goal.lt(1) ? new Decimal(1) : goal;
	}
	if (challenge_name === "dance_with_the_devil" || challenge_name === 4) {
		return toChallengeDecimal(gameData.challenges.dance_with_the_devil, 0).add(
			10.1,
		);
	}
	if (challenge_name === "legends_never_die" || challenge_name === 5) {

...
function getEssenceGain() {
	...
	const lifeIsValueable =
		milestoneData["milestone_life_is_valueable"].getEffect();
...
function getUnpausedGameSpeed() {
	...
	const speedSpeedSpeed = gameData.requirements[
		"milestone_speed_speed_speed"
	].isCompleted()
		? SPEED_SPEED_SPEED_MULTIPLIER
		: 1;
	const timeIsAFlatCircle = gameData.requirements[
		"milestone_time_is_a_flat_circle"
	].isCompleted()
...
function getLifespan() {
	...
	const speedSpeedSpeed = gameData.requirements[
		...
		: 1;
	const lifeIsValueable = gameData.requirements[
		"milestone_life_is_valueable"
	].isCompleted()
...
function isHeroesUnlocked() {
	return (
		...
		(gameData.taskData["job_one_above_all"].level >=

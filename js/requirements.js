...
function buildRequirementsFromSpec(specs) {
  ...
  for (const [key, spec] of Object.entries(specs)) {
    const selectors = spec.selectors.map(s => removeSpaces(removeStrangeCharacters(s)))
    switch (spec.type) {
      case "task":
      ...
      case "coins": {
        const reqs = spec.requirements.map(r => {
          if (r.itemRef) return { requirement: itemBaseData[r.itemRef].expense.base * (r.multiplier ?? 1) }
          return r
        })

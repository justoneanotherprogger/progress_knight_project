class Task {
	...
	querySelector(selector, row) {
		...
		if (cachedElement !== undefined) return cachedElement;
...
class Milestone {
	...
	getEffect() {
		...
		if (!gameData.requirements[this.id].isCompleted())
...
class Job extends Task {
	...
	findJobCategoryId() {
		for (const categoryId in jobCategories)
			if (this.name in jobCategories[categoryId].items) return categoryId;
		return null;
	}

	getIncome() {
		const income = toInfinityNumber(
			this.isHero ? heroIncomeMult * this.getCategoryHeroIncomeMult() : 1,
		)
...
class Skill extends Task {
	getEffect() {
		...
		switch (formula.kind) {
			case "log": {
				const logBase =
					hero && formula.logHeroBase !== undefined
						? formula.logHeroBase
						: formula.logBase || Math.E;
				const result = 1 + (value * Math.log(level + 1)) / Math.log(logBase);
				return formula.floor !== undefined
					? Math.max(result, formula.floor)
					: result;
			}
			case "linear":
				return (
					...
					level *
						value *
						(hero && formula.heroScale !== undefined ? formula.heroScale : 1)
				);
			default: {
				// power
				const levelEff = hero
					? SKILL_HERO_LEVEL_MULTIPLIER * level + SKILL_HERO_FLAT_BONUS
					: level;
				return (
					1 +
					value *
						levelEff *
						SKILL_LEVEL_EXPONENT_BASE ** getBaseLog(10, level + 1)
				);
			}
		}
	}
...
function getItemEffectDescriptionKey(target) {
	...
	switch (target.type) {
		case "happiness":
			return "effect_happiness";
		case "evil_gain":
			return "effect_evil_gain";
		case "hypercube_gain":
			return "effect_hypercube_gain";
		case "dark_matter_gain":
			return "effect_dark_matter_gain";
		case "xp": {
			const scope = target.scope || {};
			if (scope.task) return labelKey({ kind: "task", task: scope.task });
			if (scope.kind)
				return (
					labelKey({ kind: scope.kind, category: scope.category }) ||
...
class Item {
	...
	getExpense(heroic) {
		if (heroic === undefined) heroic = this.isHero;
		return toInfinityNumber(
			heroic
				? JOB_INCOME_HERO_BASE_MULTIPLIER *
						10 ** this.baseData.expense.heroExp *
						heroIncomeMult
				: 1,
		).times(
			applyMultipliers(this.baseData.expense.base, this.expenseMultipliers),
		);
	}
}
...
class EssenceRequirement extends Requirement {
	...
	getCondition(isHero, requirement) {
		...
		else return gameData.essence.gte(requirement.requirement);
	}
}
